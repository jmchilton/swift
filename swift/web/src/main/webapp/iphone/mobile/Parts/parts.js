...
dashcode.getElementSizesWithAncestor = function(A, F) {
    ...
    if (!C || C == 0) {
        ...
        while (G && (G != document)) {
            ...
            if ((I && H == "none") || (!I && H != "block")) {
                B.push({node:G,display:G.style.display});
    ...
    for (var E = 0; E < A.length; E++) {
        J.push({width:A[E].offsetWidth,height:A[E].offsetHeight})
    }
...
dashcode.getElementDocumentOffset = function(B) {
    ...
    var D = {x:B.offsetLeft,y:B.offsetTop};
    if (C) {
...
var DC = {version:"@VERSION@",revision:"@REVISION@",generateUid:(function() {
    var A = 0;
    return function() {
        return ++A
    }
})()};
DC.Browser = {IE:!!(window.attachEvent && !window.opera) && (function() {
    var B = /MSIE (\d+)/;
    var A = B.exec(navigator.userAgent);
    return A && parseInt(A[1], 10)
})(),Safari:navigator.userAgent.indexOf("AppleWebKit/") > -1,Safari2:(function() {
    var A = /AppleWebKit\/(\d+(?:\.\d+)?)/;
    var B = A.exec(navigator.userAgent);
    return(B && parseInt(B[1], 10) < 420)
})(),Mozilla:navigator.userAgent.indexOf("Gecko") > -1 && navigator.userAgent.indexOf("KHTML") == -1,MobileSafari:!!navigator.userAgent.match(/Apple.*Mobile.*Safari/)};
DC.Support = {Properties:("__defineGetter__" in Object.prototype),QuerySelector:("querySelector" in document),Touches:!!document.createTouch,CSS3ColorModel:false,CSSTransitions:false,BorderImage:(function() {
...
DC.Error = Class.create({constructor:function(A) {
    Object.extend(this, A)
},description:_("error.no_description"),recoverySuggestion:null});
DC.KeyInfo = Class.create({constructor:function(D, B) {
...
DC.ChangeType = {setting:0,insertion:1,deletion:2,replacement:3};
DC.ChangeNotification = Class.create({constructor:function(D, A, E, C, B) {
    this.object = D;
    this.changeType = A;
    this.newValue = E;
    this.oldValue = C;
...
DC.ObserverEntry = Class.create({constructor:function(A, C, B) {
    this.observer = A;
    this.callback = C;
    this.context = B
...
DC.KVO = Class.create({constructor:function() {
...
DC.KVO.getPropertyMethodsForKeyOnObject = (function() {
    ...
    function E(T, K) {
        ...
        return{getter:O,mutator:H,validator:F,value:S}
    }
    ...
    function A(Q, J) {
        ...
        return{getter:M,mutator:G,validator:F,value:P}
    }
...
DC.Bindable = Class.create(DC.KVO, {constructor:function(A) {
    this.bindings = {};
    this.__parameters = A;
    ...
    DC.dataModel = A;
    this.__context = C
},exposedBindings:[],defaultPlaceholders:{},automaticallySetupBindings:true,__relativeSource:null,defaultPlaceholderForMarkerWithBinding:function(B, C) {
    ...
    if ("transformValue" in E) {
        E.transformer = {transformValue:E.transformValue,reverseTransformedValue:E.reverseTransformedValue || null};
...
DC.SortDescriptor = Class.create({constructor:function(D, A, B) {
    this.keyPath = D;
    this.ascending = A;
...
DC.ValueTransformer = Class.create({transformedValue:function(A) {
    return A
...
DC.transformer.Not = Class.create(DC.ValueTransformer, {transformedValue:function(A) {
    return(A ? false : true)
},reverseTransformedValue:function(A) {
    return !!A
}});
DC.registerTransformerWithName(new DC.transformer.Not(), "not");
DC.transformer.Boolean = Class.create(DC.ValueTransformer, {constructor:function(A, B) {
    this.trueValue = A;
    this.falseValue = B
...
DC.transformer.Matches = Class.create(DC.ValueTransformer, {constructor:function(A) {
    this.trueRegex = A
},transformedValue:function(A) {
    return this.trueRegex.test(A)
}});
DC.transformer.Generic = Class.create(DC.ValueTransformer, {constructor:function(A, B) {
    this.modelValues = A;
...
DC.transformer.Truncated = Class.create(DC.ValueTransformer, {constructor:function(A) {
    this.max = A || 50
},ellipsis:String.fromCharCode(8230),transformedValue:function(C) {
    if (!C && 0 !== C) {
        return C
    }
    C = "" + C;
...
DC.transformer.StringsToObjects = Class.create(DC.ValueTransformer, {constructor:function(A) {
    this.key = A || "string"
},transformedValue:function(B) {
    if (!B || !B.map) {
        return B
...
DC.transformer.FirstObject = Class.create(DC.ValueTransformer, {transformedValue:function(A) {
    if (DC.typeOf(A) == "array") {
        return A[0]
    }
    return A
}});
Object.extend(DC, {NoSelectionMarkerType:"noSelection",MultipleValuesMarkerType:"multipleValues",NullValueMarkerType:"nullValue"});
DC.Binding = Class.create({constructor:function(A) {
    Object.extend(this, A);
...
Class.extend(Array, {containsObject:function(A) {
...
DC.ArrayOperator = {avg:function(A) {
...
Object.extend(DC, {dataModel:new DC.KVO(),registerModelWithName:function(B, A) {
...
(function() {
    ...
    var A = 0;
    var B = 1;
    DC.Deferred = Class.create({constructor:function(D) {
        this.canceller = D;
        this._result = null;
        this._status = C;
        this._callbacks = []
...
window.XHR = (function() {
    ...
    return{get:function(url, parameters, options) {
...
DC.Controller = Class.create(DC.Bindable, {constructor:function(A) {
    this.base(A)
...
DC.Markers = {MultipleValues:"ThisIsAnUniqueStringThatRepresentsMultipleValues",NoSelection:"ThisIsAnUniqueStringThatRepresentsNoSelection"};
DC.SelectionProxy = Class.create(DC.KVO, {constructor:function(A) {
    this.controller = A;
    this.mutable = true
...
DC.ObjectController = Class.create(DC.Controller, {constructor:function(A) {
    this.base(A);
...
},keyDependencies:{},exposedBindings:["editable","content"],editable:function() {
...
DC.ArrayController = Class.create(DC.ObjectController, {constructor:function(A) {
    this.base(A);
    this.arrangedObjects = [];
    this.__sortDescriptors = [];
    this.__filterPredicate = null;
    this.__selectionIndexes = []
},keyDependencies:{selectedObjects:["selectionIndexes"],selectionIndex:["selectionIndexes"],hasSelection:["selectionIndexes"],canRemove:["editable","selectionIndexes"],canAdd:["editable"]},selectsInsertedObjects:true,exposedBindings:["selectionIndexes","sortDescriptors","filterPredicate","contentForMultipleSelection"],observeContentChange:function(D) {
    var A;
    var C;
    switch (D.changeType) {
        case DC.ChangeType.setting:
        ...
        case DC.ChangeType.insertion:
            ...
            if (this.selectsInsertedObjects && !this.selectedObjects()) {
                ...
                if (!this.selectionIndexes().length && this.avoidsEmptySelection()) {
    ...
    B = B || [];
    B.sort();
    if ((0 === (this.selectionIndexes() || []).compare(B)) && !this._inReArrangeObjects) {
        return false
    }
...
DC.AjaxController = Class.create(DC.ObjectController, {flushContentBeforeQuery:false,fetchesInitially:true,constructor:function(A) {
    this.queryDelay = 0;
    this.url = "";
    this.method = "GET";
    this.base(A);
    ...
    this.__request = XHR.get(this.url, F, {responseContentType:B});
...
DC.ModeledXMLProxy = Class.create(DC.KVO, {_xmlRoot:null,_xmlNode:null,constructor:function(A, C, B) {
    this._xmlRoot = A;
    this._xmlNode = C;
    this._xmlModel = B;
    this._valueCache = {};
...
DC.ModeledXMLKeyInfo = Class.create(DC.KeyInfo, {constructor:function(D, C) {
    ...
    DC.KVO.getPropertyMethodsForKeyOnObject = function(E, F) {
        return{value:B,getter:function() {
...
Object.extend(Element, {uniqueId:function() {
    ...
    B.position = "absolute";
    B.display = "block";
    var D = {width:E.offsetWidth,height:E.offsetHeight,left:E.offsetLeft,top:E.offsetTop};
    B.display = A;
    B.position = C;
    B.visibility = F;
    return D
    ...
    var A = document.body;
    return{left:window.scrollLeft || B.scrollLeft || A.scrollLeft,top:window.scrollTop || B.scrollTop || A.scrollTop,width:window.innerWidth || B.clientWidth || A.clientWidth,height:window.innerHeight | B.clientHeight || A.clientHeight}
    ...
    if (P != document.body && P.getBoundingClientRect) {
        ...
        L = {left:L.left,right:L.right,top:L.top,bottom:L.bottom};
        if (!!N === false) {
    ...
    return{left:D,top:O,right:D + G.width - 1,bottom:O + G.height - 1,width:G.width,height:G.height}
}});
Element.getStyle = Element.getStyles;
Element.assignId.uniqueId = 1;
if (!DC.Support.QuerySelector) {
    Object.extend(Element, {query:function(B, A) {
        if (1 == arguments.length) {
            A = B;
            B = document
        }
...
Object.extend(Event, {KEY_BACKSPACE:8,KEY_TAB:9,KEY_RETURN:13,KEY_ESC:27,KEY_LEFT:37,KEY_UP:38,KEY_RIGHT:39,KEY_DOWN:40,KEY_DELETE:46,KEY_HOME:36,KEY_END:35,KEY_PAGEUP:33,KEY_PAGEDOWN:34,KEY_INSERT:45,_domHasFinishedLoading:function() {
...
DC.PartFinder = (function() {
    ...
    function B(E, D) {
        ...
        return{nodes:F,ids:G}
    }

    return{singlePart:function(E, D) {
        function F() {
            var K = D;
            var G = typeof (K);
            if ("function" === G) {
...
DC.Responder = Class.create(DC.Bindable, {sendActionToView:function(B, A) {
...
DC.View = Class.create(DC.Responder, {exposedBindings:["visible","class","enabled","editable","html","text"],defaultPlaceholders:{text:{multipleValues:_("marker.text.multipleValues"),nullValue:_("marker.text.placeholder"),noSelection:_("marker.text.noSelection")},html:{multipleValues:_("marker.text.multipleValues"),nullValue:_("marker.text.placeholder"),noSelection:_("marker.text.noSelection")}},automaticallySetupBindings:false,formatter:null,target:null,action:null,sendActionOn:["click"],animated:false,updateAnimationDuration:0,updateAnimationDelay:0,postUpdateAnimationDuration:0,postUpdateAnimationDelay:0,preUpdateAnimationDuration:0,preUpdateAnimationDelay:0,constructor:function(A, B, C) {
    ...
    if (!this.animated) {
        ...
        if (F.newValue) {
            ...
            DC.Animator.removeClassName(A, D, {duration:this.updateAnimationDuration || 300})
        } else {
            DC.Animator.addClassName(A, D, {duration:this.updateAnimationDuration || 300,callback:C})
        }
    }
    ...
    var B = Set.join(F, " ");
    if (this.animated) {
        DC.Animator.setClassName(A, B, {duration:500})
    } else {
        A.className = B
    }
...
DC.DashcodePart = Class.create(DC.View, {maskedBindings:["html","text"],constructor:function(A, C, D, B) {
...
DC.Text = Class.create(DC.DashcodePart, {exposedBindings:["text","html"],partSetup:function(A) {
    if (A.text) {
        var B = A.text;
...
DC.DashcodeOverrides = {Original:{View:{createViewForNode:DC.View.createViewForNode,viewClassForNode:DC.View.viewClassForNode},Binding:{bindingFromString:DC.Binding.bindingFromString}},Replacement:{View:{createViewForNode:function(B, C, A) {
    ...
    return viewClass
}},Binding:{bindingFromString:function(B, A) {
...
DC.ListView = Class.create(DC.DashcodePart, {exposedBindings:["content","selectionIndexes","selectedIndex","selectedObject"],allowsEmpty:true,_items:null,_activeItem:-1,maximumCachedRows:30,init:function() {
    this.base();
    var B = this.viewElement();
    var A;
    switch (B.tagName) {
        case"SELECT":
            ...
            Event.observe(B, "change", this.selectedIndexChanged.bindAsEventListener(this));
            break;
        case"TABLE":
        ...
        default:
            ...
            DC.View.createViewsForNodeTree(C, null, this.__context);
            C.parentNode.removeChild(C);
            this._items().forEach(this.removeChild, this);
            this._items.removeAll();
            break
    }
    if ("true" === (B.getAttribute("multiple") || "").toLowerCase()) {
        this.multiple = true
    }
    ...
    var M;
    if (this.animated) {
        var N = {curve:DC.easing.inOutSine,duration:300};
        I = function(P, O) {
    ...
    if (!C) {
        var H = this.__selectionIndexes ? this.__selectionIndexes.length : 0;
        for (F = 0; F < H; F++) {
    ...
    return A.objectsAtIndexes(this.selectionIndexes() || [])
},onkeydown:function(B) {
    var J = this.viewElement();
    if (J.disabled) {
        return
    }
    if (Event.KEY_UP != B.keyCode && Event.KEY_DOWN != B.keyCode) {
        return
    }
    Event.stop(B);
    ...
    if (this.multiple && B.shiftKey && I.length) {
        ...
        if (!I.length) {
            ...
            if (Event.KEY_UP == B.keyCode && this.selectedIndex > 0) {
                this.selectedIndex--
            } else {
                if (Event.KEY_DOWN == B.keyCode && this.selectedIndex < E) {
                    this.selectedIndex++
                }
            }
            I = [L = this.selectedIndex]
    ...
    switch (L.changeType) {
        ...
        case DC.ChangeType.insertion:
            ...
            for (H = 0; H < L.indexes.length; ++H) {
                ...
                if (M && "none" === M.style.display) {
                    ...
                    I = this.createElement(L.newValue[H], M, null, L.indexes[H]);
                    this._items.insertPartAtIndex(I, L.indexes[H])
                }
                if (this.animated) {
                    Element.addClassName(I, DC.Style.kInsertedClass);
                    DC.Animator.removeClassName(I, DC.Style.kInsertedClass, {discreteTransitionPoint:1,duration:this.updateAnimationDuration})
                }
            }
            break;
        case DC.ChangeType.replacement:
        function E(R) {
            ...
            DC.Animator.removeClassName(R, DC.Style.kReplacingClass, {duration:250,discreteTransitionPoint:1})
        }
            ...
            for (H = 0; H < L.indexes.length; ++H) {
                ...
                if (!this.animated) {
                    ...
                    DC.Animator.addClassName(I, DC.Style.kReplacingClass, {duration:this.updateAnimationDuration,discreteTransitionPoint:0,callback:E.bind(this)})
                }
            }
            break;
        case DC.ChangeType.deletion:
            ...
            for (H = L.indexes.length - 1; H >= 0; --H) {
                ...
                if (!this.animated) {
                    ...
                    DC.Animator.addClassName(I, DC.Style.kDeletedClass, {duration:this.updateAnimationDuration,discreteTransitionPoint:0,callback:this.removeChild.bind(this)})
                }
            }
    ...
    if (this.__content && this.__content.length) {
        if (K[0] != D) {
            Element.addClassName(K[0], DC.Style.kFirstClass);
            Element.removeClassName(D, DC.Style.kFirstClass)
        }
        if (J != (this.__content.length - 1)) {
            Element.addClassName(K[this.__content.length - 1], DC.Style.kLastClass);
            if (J < K.length) {
                Element.removeClassName(K[J], DC.Style.kLastClass)
            }
        }
    } else {
        if (K.length) {
            Element.removeClassName(D, DC.Style.kFirstClass);
            if (J < K.length) {
                Element.removeClassName(D, DC.Style.kLastClass)
            }
        }
    }
    ...
    if (this.template) {
        ...
        DC.View.createViewsForNodeTree(D, C, this.__context)
    }
    return D
    ...
    if (this.template) {
        ...
        E = DC.View.cloneViewsForTreeNode(this.templateElement, B, this.__context);
        if (!H) {
            var A = C.children;
            var D = null;
            if (A && A.length) {
...
DC.EventLoop = {currentEvent:null,getStart:function() {
    if (!this._start) {
...
DC.Page = Class.create(DC.Responder, {focusedElement:null,constructor:function() {
    ...
    if ("string" === typeof (C)) {
        this._delegates[C].push({sel:A,fn:B})
    } else {
        var D;
        for (D in C) {
            ...
            this._delegates[D].push({sel:A,fn:C[D]})
        }
    }
},superview:function() {
    return null
...
var dashcodePartSpecs = {
    ...
    "detailTitle": { "propertyValues": { "textBinding": { "keypath": "list.selection.title" } }, "text": "Title", "view": "DC.Text" },
    "detailUser": { "propertyValues": { "textBinding": { "keypath": "list.selection.user" } }, "text": "User", "view": "DC.Text" },
    "div": { "baseImagesPath": "..", "criticalValue": 0.5, "propertyValues": { "valueBinding": { "keypath": "list.selection.failures" } }, "view": "DC.Indicator", "warningValue": 0.1 },
    "end": { "propertyValues": { "textBinding": { "keypath": "list.selection.duration" } }, "text": "30:20", "view": "DC.Text" },
    "endLabel": { "text": "Duration", "view": "DC.Text" },
    "endLabel1": { "text": "Ok", "view": "DC.Text" },
    "endLabel2": { "text": "Warn", "view": "DC.Text" },
    "endLabel3": { "text": "Fail", "view": "DC.Text" },
    "endLabel4": { "propertyValues": { "textBinding": { "keypath": "list.selection.ok" } }, "text": "10", "view": "DC.Text" },
    "endLabel5": { "propertyValues": { "textBinding": { "keypath": "list.selection.warnings" } }, "text": "0", "view": "DC.Text" },
    "endLabel6": { "propertyValues": { "textBinding": { "keypath": "list.selection.failures" } }, "text": "10", "view": "DC.Text" },
    "header": { "rootTitle": "Swift Status", "view": "DC.Header" },
    "indicator": { "baseImagesPath": "..", "criticalValue": 0.8, "propertyValues": { "valueBinding": { "keypath": "*.failures" } }, "view": "DC.Indicator", "warningValue": 0.5 },
    ...
    "rowTitle": { "propertyValues": { "textBinding": { "keypath": "*.title" } }, "text": "Item", "view": "DC.Text" },
    "stackLayout": { "subviewsTransitions": [
        { "direction": "right-left", "duration": "", "timing": "ease-in-out", "type": "push" },
        { "direction": "right-left", "duration": "", "timing": "ease-in-out", "type": "push" }
    ], "view": "DC.StackLayout" },
    "start": { "propertyValues": { "textBinding": { "keypath": "list.selection.submitted" } }, "text": "10:20:30", "view": "DC.Text" },
    "startLabel": { "text": "Start", "view": "DC.Text" }
};
...
var dashcodeDataSources = {
    "dataSource": { "Class": "DC.AjaxController", "propertyValues": { "url": "/status/searches", "parameters": {}, "dataModel": { "uuid": "14E46AE4-B98C-4B96-851A-739577A3D48E", "url": null, "dataType": "json", "version": 1, "root": "", "model": [
        { "_": 1, "id": 1, "title": "", "user": "", "submitted": "", "duration": "", "subtasks": 1, "search": 1, "ok": 1, "failures": 1, "warnings": 1, "running": 1, "details": { "total": 1 } }
    ] } } }
};

DC.Browser = Class.create(DC.DashcodePart, {__viewClassName__:"Browser",partSetup:function(B) {
    this.base(B);
    this._clearSelectionOnBack = B.clearSelectionOnBack || false;
    this._topPosFromBody = 0;
    var A = this.viewElement();
    do{
        this._topPosFromBody += A.offsetTop
    } while (A = A.offsetParent)
},finishLoading:function() {
    for (var A = this.viewElement().lastChild; A != null; A = A.previousSibling) {
        if (A.nodeType != 1) {
            continue
        }
        if (!this._contentElement) {
            if (A.object instanceof DC.StackLayout) {
                this._contentElement = A;
                this._contentElement.object._setRestrictToBrowserTransition(true)
            }
        } else {
            if (!this._headerElement) {
                if (A.object instanceof DC.Header) {
                    this._headerElement = A
                }
            }
        }
    }
...
},goForward:function(I, E, A) {
    var H = this;
    var C = this._contentElement.object.getCurrentView();
    var G = function() {
        var K = H._contentElement.object.getTransitionForView(I);
        if (K) {
            var J = (K.type != Transition.NONE_TYPE) ? parseFloat(K.getDuration()) : 0;
            H._headerElement.object._setAnimationDuration(J);
            H._headerElement.object._setAnimationTiming(K.timing)
        }
        if (A) {
            A()
        }
        if (H.willGoToViewCallback) {
            H.willGoToViewCallback(H, C, I, true)
        }
        H._contentElement.object._privateEndTransitionCallback = function(N, L, M) {
            H._navigationDidEnd(L, M, true)
        };
        H._backButtonClicked(C)
    };
    var D = this._contentElement.object.getTransitionForView(I);
    if (D) {
        var B = (D.type != Transition.NONE_TYPE) ? parseFloat(D.getDuration()) : 0;
        this._headerElement.object._setAnimationDuration(B);
        this._headerElement.object._setAnimationTiming(D.timing)
    }
    var F = this._topPosFromBody - window.pageYOffset;
    if (F < 0) {
        window.scrollBy(0, F)
    }
    if (this.willGoToViewCallback) {
        this.willGoToViewCallback(this, C, I, false)
    }
    var H = this;
    this._contentElement.object._privateEndTransitionCallback = function(L, J, K) {
        H._navigationDidEnd(J, K, false)
    };
    this._contentElement.object._maskContainerElement = this.viewElement();
    this._contentElement.object.setCurrentView(I);
    this._headerElement.object._goForward(E, G)
},goBack:function() {
    return this._headerElement.object._goBack()
},getClearSelectionOnBack:function() {
    return this._clearSelectionOnBack
},setClearSelectionOnBack:function(A) {
    this._clearSelectionOnBack = A
},_backButtonClicked:function(A) {
    this._contentElement.object._maskContainerElement = this.viewElement();
    this._contentElement.object.setCurrentView(A, true)
},_navigationDidEnd:function(B, A, C) {
    if (C && this._clearSelectionOnBack) {
        var D = A.firstChild;
        while (D) {
            if (D.object && D.object.clearSelection) {
                D.object.clearSelection(true)
            }
            D = D.nextSibling
        }
    }
    if (this.didGoToViewCallback) {
        this.didGoToViewCallback(this, B, A, C)
    }
}});
DC.Header = Class.create(DC.DashcodePart, {__viewClassName__:"Header",exposedBindings:[],partSetup:function(H) {
    var B = this.viewElement();
    var E = this.viewElement();
    var F = B.children[0];
    if (H.originalID) {
        E = document.getElementById(H.originalID)
    }
    this._headerHeight = dashcode.getElementHeight(E) || 20;
    ...
    for (var C = 0; C < B.children.length; C++) {
        ...
        if (D && D.indexOf("dashcode-header-backbutton") > -1) {
            this._backButtonTemplate = A
        }
    }
    ...
    B.insertBefore(this._topSeparatorElement, F);
    B.insertBefore(this._bottomSeparatorElement, F);
    B.insertBefore(this._title1, F);
    B.insertBefore(this._title2, F);
    if (!this._inDesign) {
        this._backButtonTemplate.style.display = "none";
        B.insertBefore(this._backButton1, F);
        B.insertBefore(this._backButton2, F)
    }
    var I = this;
    this._backButton1.object.action = this._backButton2.object.action = function(J, K) {
        I._goBack()
    };
    ...
    this._stack = [];
    var G = H.rootTitle || "Home";
    this._goForward(G, null);
    Event.observe(window, "orientationchange", function() {
        I._sizeChanged()
    })
},_makeBackButton:function(B) {
    var A = dashcode.cloneTemplateElement(B, false);
    A.style.opacity = 0;
    if (this._useTransforms) {
        A.style.webkitTransform = this._translateOp(0, 0)
    }
    return A
},_makeTitle:function() {
    var B = document.createElement("div");
    var A = this._headerHeight;
    B.style.position = "absolute";
    B.style.top = 0;
    B.style.height = A + "px";
    if (this._useTransforms) {
        B.style.webkitTransform = this._translateOp(0, 0)
    }
    B.style.lineHeight = A + "px";
    B.style.textAlign = "center";
    B.style.textOverflow = "ellipsis";
    B.style.whiteSpace = "nowrap";
    B.style.overflow = "hidden";
    B.style.opacity = 0;
    B.setAttribute("class", "apple-hidden");
    return B
},_makeSeparator:function(C) {
    var A = document.createElement("div");
    var B = A.style;
    B.position = "absolute";
    B.left = "0px";
    ...
    if (C == "TOP") {
        B.top = "0px";
        B.backgroundColor = DC.Support.CSS3ColorModel ? "rgba(255, 255, 255, 0.4)" : "white"
    } else {
        B.bottom = "0px";
        B.backgroundColor = DC.Support.CSS3ColorModel ? "rgba(0, 0, 0, 0.6)" : "black"
    }
    A.setAttribute("class", "apple-hidden");
    return A
},_disableTransitions:function(B) {
    if (this._useTransforms) {
        for (var A = 0; A < B.length; A++) {
            B[A].style.webkitTransitionProperty = "none"
        }
    }
},_enableTransitions:function(B) {
    if (this._useTransforms) {
        for (var A = 0; A < B.length; A++) {
            B[A].style.webkitTransitionProperty = this._animationProperties;
            B[A].style.webkitTransitionDuration = this._animationDurations;
            B[A].style.webkitTransitionTimingFunction = this._animationTimings
        }
    }
},_translateOp:function(A, B) {
    return"translate(" + A + "px, " + B + "px)"
},_setAnimationDuration:function(A) {
    if (this._useTransforms) {
        this._animationDuration = A;
        this._animationDurations = A + "s, " + A + "s"
    }
},_setAnimationTiming:function(A) {
    if (this._useTransforms) {
        this._animationTimings = A + ", linear"
    }
},_sizeChanged:function() {
    var H = this._stack[this._stack.length - 1];
    var G = H.titleElement;
    var F = H.backButtonElement;
    var C = this.viewElement();
    G.style.width = "";
    var D = [C,F,G];
    var L = dashcode.getElementSizesWithAncestor(D, C);
    var K = L[0].width;
    var I = L[1].width;
    var B = L[2].width;
    var E = H.backButtonX + I;
    var J = this._paddingSides + ((E > 0) ? E + this._paddingSides : 0);
    var A = this._adjustedTitlePosition(B, K, J);
    this._disableTransitions([G]);
    if (A.width != null) {
        G.style.width = A.width + "px"
    }
    if (this._useTransforms) {
        G.style.webkitTransform = this._translateOp(A.x, 0)
    } else {
        G.style.left = A.x + "px"
    }
    H.titleX = A.x
},_adjustedTitlePosition:function(E, C, B) {
    var A = {};
    A.x = (C / 2) - (E / 2);
    if (B > A.x) {
        A.x = B;
        var D = C - A.x - this._paddingSides;
        if (E > D) {
            A.width = D
        }
    }
    return A
},_performTransition:function(C) {
    var D = C.length;
    for (var E = 0; E < D; E++) {
        var A = C[E];
        var B = A.element;
        if (this._useTransforms) {
            B.style.webkitTransform = this._translateOp(A.translateX, 0)
        } else {
            B.style.left = A.translateX + "px"
        }
        if (A.opacity != null) {
            B.style.opacity = A.opacity
        }
    }
},_goForward:function(B, A) {
    this._navigate(true, B, A)
},_goBack:function() {
    this._navigate(false)
},_navigate:function(E, a, b) {
    var B = this.viewElement();
    if (!E && this._stack.length < 2) {
        return
    }
    var c,X,C,T;
    ...
    var U = null,Z = null,e = null;
    if (E) {
        if (this._stack.length > 0) {
            U = this._stack[this._stack.length - 1]
        }
    } else {
        Z = this._stack.pop();
        e = this._stack[this._stack.length - 1];
        if (this._stack.length > 1) {
            U = this._stack[this._stack.length - 2]
        }
        a = e.title
    }
    this._title = a;
    C.object.setText(U ? U.title : "");
    C.object.sizeToFit(40, 100);
    c.style.width = "";
    c.innerHTML = a;
    var Q = [B,c];
    if (U) {
    ...
    var A = dashcode.getElementSizesWithAncestor(Q, B);
    var J = A[0].width;
    var P = A[1].width;
    var N = U ? A[2].width : J;
    var H = U ? A[3].width : J;
    var F = U ? this._paddingSides : -J;
    var S = F + N;
    var d = this._paddingSides + ((S > 0) ? S + this._paddingSides : 0);
    var K = this._adjustedTitlePosition(P, J, d);
    var W = K.x;
    if (K.width != null) {
        c.style.width = K.width + "px"
    }
    var D,M;
    if (E) {
        D = this._paddingSides + this._backButtonLeftPadding;
        if (U) {
            D = Math.min(D, U.titleX)
        }
        M = -(H + this._paddingSides)
    } else {
        D = J;
        M = Math.max(this._paddingSides, W - this._backButtonLeftPadding)
    }
    this._disableTransitions([c,X,C,T]);
    C.style.opacity = 0;
    ...
    if (E) {
        if (U) {
            V = Math.max(F, U.titleX - this._backButtonLeftPadding);
            I = J
        } else {
            V = F;
            I = W;
        ...
        V = -(N + this._paddingSides);
        I = this._paddingSides + this._backButtonLeftPadding;
        I = Math.min(I, W)
    }
    c.style.opacity = Y;
    if (this._useTransforms) {
        c.style.webkitTransform = this._translateOp(I, 0);
        C.style.webkitTransform = this._translateOp(V, 0)
    }
    if (this._inDesign) {
        W = this._useTransforms ? 0 : this._paddingSides;
        c.style.width = "";
        c.style.left = this._paddingSides + "px";
        c.style.right = this._paddingSides + "px"
    }
    var O = [
        {element:c,translateX:W,opacity:1},
        {element:X,translateX:D,opacity:0},
        {element:C,translateX:F,opacity:U != null ? 1 : 0},
        {element:T,translateX:M,opacity:0}
    ];
    var R = this;
    Transition._addDelayedTransitionCallback(function() {
        if (G) {
            R._enableTransitions([c,C,X,T])
        }
        R._performTransition(O)
    });
    this._checkedForEnded = false;
    this._vanishButton = T;
    Event.observe(T, "webkitTransitionEnd", this);
    if (E) {
        var L = {title:a,titleX:W,backButtonX:F,titleElement:c,backButtonElement:C,callback:b};
        this._stack.push(L)
    } else {
        if (Z.callback) {
            Z.callback(Z.title, e.title)
        }
    }
},_getCurrentTitle:function(A) {
    return this._title
...
DC.List = Class.create(DC.ListView, {__viewClassName__:"DashcodeList",selectsInsertedItems:false,maskedBindings:["content","selectionIndexes","selectedIndex","selectedObject"],exposedBindings:["dataArray"],partSetup:function(spec) {
    ...
    if (A) {
        A.setAvoidsEmptySelection(!this.allowsEmpty && this._selectionEnabled && !dashcode.inDesign)
    }
            ...
            if (this._useDataSource && this.dataSource) {
                ...
                for (var G = 0; G < B; G++) {
                    ...
                    if (this.dataSource.objectForRow) {
                        F = this.dataSource.representationForRow(G);
    ...
    if (!dashcode.inDesign && E && E.length && this.getSelectionEnabled() && !this.getAllowsEmptySelection()) {
    ...
    if (typeof (AppleScrollArea) != "undefined") {
        ...
        while (D && D != document) {
            if (D.object && D.object.constructor == AppleScrollArea) {
                D.object.refresh();
                break
            }
            D = D.parentNode
    ...
    if (dashcode.inDesign && (C == 0) && (E != this.templateElement)) {
        A = A || this.container();
        ...
        DC.View.createViewsForNodeTree(E, null, this.__context);
    ...
    if (this._useDataSource) {
        if (this.dataSource && this.dataSource.prepareRow) {
            this.dataSource.prepareRow(D, C, D.object.templateElements)
        }
    } else {
        if (this.dataArray) {
            ...
            this._prepareStaticRow(D, C, D.object.templateElements)
        }
    }
    if (D.onclick) {
        var B = this;
    ...
    if (DC.Support.Touches) {
        ...
        if (!A) {
            A = this._findRow(B.target ? B.target : B.srcElement)
        }
        if (A && !A.object._highlighted && this._selectionEnabled) {
    ...
    if (!this.action) {
        ...
        if (A && A.object && A.object.clickHandler) {
            this._sendLegacyOnClick(B, A, A.object.clickHandler)
        }
    }
    delete this._temporaryHighlight
},onswipe:function(B) {
    if (this.dataSource && this.dataSource.rowSwiped) {
        var A = this._findRow(B.target ? B.target : B.srcElement);
    ...
    for (var J in I) {
        if (this._labelElementId && this._labelElementId.length > 0 && J.indexOf(this._labelElementId) == 0) {
            F = I[J];
            break
        }
    }
    if (this._labelElementId && this._labelElementId.length && (E == this.templateElement)) {
        var G = Element.queryAll(E, "#" + this._labelElementId);
        if (G.length > 0) {
            F = G[0]
        }
    }
        ...
        if (D) {
            if (this._rowHighlight) {
                if (!H.object.rowHighlight) {
                    H.object.rowHighlight = this._rowHighlight.cloneNode(true);
                    var I = H.object.index;
                    if (this._listStyle == DC.List.ROUNDED_RECTANGLE && (I == 0 || I == H.parentNode.children.length - 1)) {
                        var F = Element.getStyles(this.viewElement(), ["border-top-width","border-bottom-width","-webkit-border-top-left-radius","-webkit-border-top-right-radius","-webkit-border-bottom-left-radius","-webkit-border-bottom-right-radius"]);
                        var E = H.object.rowHighlight.style;
                        if (I == 0) {
                            var A = parseInt(F["border-top-width"], 10);
                            E.webkitBorderTopLeftRadius = (parseInt(F["-webkit-border-top-left-radius"], 10) - A) + "px";
                            E.webkitBorderTopRightRadius = (parseInt(F["-webkit-border-top-right-radius"], 10) - A) + "px"
                        }
                        if (I == H.parentNode.children.length - 1) {
                            var A = parseInt(F["border-bottom-width"], 10);
                            E.webkitBorderBottomLeftRadius = (parseInt(F["-webkit-border-bottom-left-radius"], 10) - A) + "px";
                            E.webkitBorderBottomRightRadius = (parseInt(F["-webkit-border-bottom-right-radius"], 10) - A) + "px"
                        }
                    }
                }
                ...
                Element.addClassName(H, DC.Style.kSelectedClass)
            }
        } else {
            var C = H.object.rowHighlight;
            if (C) {
                if (G) {
                    ...
                    var B = function() {
                        H.removeChild(C);
                        Event.stopObserving(C, "webkitTransitionEnd", arguments.callee)
                    };
                    Event.observe(C, "webkitTransitionEnd", B);
                    window.setTimeout(function() {
                        C.style.opacity = 0
                    }, 0)
                } else {
                ...
                Element.removeClassName(H, DC.Style.kSelectedClass)
            }
        }
...
;
DC.StackLayout = Class.create(DC.DashcodePart, {__viewClassName__:"StackLayout",exposedBindings:["currentView"],clonedFrom:function(A) {
    this.base(A);
    ...
    this.setCurrentViewWrapped(A, arguments[1], arguments[2])
    ...
    this._restrictedBrowserTransition = A ? CreateTransitionWithProperties({type:Transition.PUSH_TYPE,direction:Transition.RIGHT_TO_LEFT_DIRECTION,timing:Transition.EASE_IN_OUT_TIMING}) : null
...
Transition._DEFAULT_DURATION = {none:"0.35",push:"0.35",dissolve:"0.35",slide:"0.35",fade:"0.35",flip:"0.65",cube:"0.55",swap:"0.55",revolve:"0.35"};
    ...
    if (this._oldView) {
        ...
        Transition._addDelayedTransitionCallback(function() {
            ...
            if (B || (F && A)) {
                ...
                I._setupTransition(C, "-webkit-transform", L, I.timing, "webkitTransform", K)
            } else {
                I._shouldHideOldView = false
            }
            if (B || (F && !A)) {
                I._setupTransition(E, "-webkit-transform", L, I.timing, "webkitTransform", I._translateOp(0, 0))
            }
        });
        Event.observe(this._newView, "webkitTransitionEnd", this)
    }
...
Transition.prototype._performFlipTransition = function(B) {
    if (this._oldView) {
        ...
        if (Transition._containerFlipTranslateZStyle) {
            Transition._containerFlipTranslateZStyle.webkitTransform = "translateZ(" + -1 * G / 2 + "px)"
        }
        if (!this._pushContainer) {
        ...
        if (K != Transition.RIGHT_TO_LEFT_DIRECTION && K != Transition.LEFT_TO_RIGHT_DIRECTION) {
            K = Transition.RIGHT_TO_LEFT_DIRECTION
        }
        var A = ((K == Transition.RIGHT_TO_LEFT_DIRECTION) && !B) || ((K == Transition.LEFT_TO_RIGHT_DIRECTION) && B);
        E.webkitAnimationDuration = M;
        E.webkitAnimationTimingFunction = this.timing;
        var L = A ? "dashcode-transition-flip-right-old-view" : "dashcode-transition-flip-left-old-view";
        Transition._addClassName(this._oldView, L);
        H.webkitAnimationDuration = M;
        H.webkitAnimationTimingFunction = this.timing;
        var F = A ? "dashcode-transition-flip-right-new-view" : "dashcode-transition-flip-left-new-view";
...
Transition.prototype._performCubeTransition = function(A) {
    if (this._oldView) {
        ...
        Transition._addDelayedTransitionCallback(function() {
            ...
            G._setupTransition(C, "-webkit-transform", J, G.timing, "webkitTransform", G._rotateOp("Y", F * -90) + " translateZ(" + D + "px)");
            G._setupTransition(E, "-webkit-transform", J, G.timing, "webkitTransform", "rotateY(0deg) translateZ(0px)")
        });
        Event.observe(this._newView, "webkitTransitionEnd", this)
    }
};
...
DC.PushButton = Class.create(DC.DashcodePart, {exposedBindings:["sizeToFit","state","text"],__viewClassName__:"PushButton",partSetup:function(spec) {
    ...
    if (A) {
        B.style.opacity = D * (1 / DC.PushButton.DISABLED_OPACITY);
        B.style.appleDashboardRegion = "dashboard-region(control rectangle)"
    } else {
        B.style.opacity = DC.PushButton.DISABLED_OPACITY * D;
    ...
    var A = B ? DC.PushButton.STATE_ON : DC.PushButton.STATE_OFF;
    this.setState(A)
    ...
    if (this._noBackground) {
        ...
        if (this._leftImageWidth > 0 || this._rightImageWidth > 0) {
            ...
            C.backgroundImage = "url(" + this._pressedBgImageURL + ")";
    ...
    if (!this._customImage || (!this._customImageLoaded && !this._customImagePressedLoaded && (R != DC.PushButton.IMAGE_POSITION_CENTER))) {
    ...
    if (R == DC.PushButton.IMAGE_POSITION_NONE) {
        ...
        if (A < 1) {
            var O = Element.getStyles(this._textElement, ["font-size"]);
        ...
        if (R == DC.PushButton.IMAGE_POSITION_LEFT || R == DC.PushButton.IMAGE_POSITION_RIGHT) {
            E.display = "inline";
            J.display = "inline";
            J.marginTop = "";
            J.marginBottom = "";
            ...
            if (R == DC.PushButton.IMAGE_POSITION_TOP || R == DC.PushButton.IMAGE_POSITION_BOTTOM) {
                E.display = "inline";
                J.display = "inline";
                J.marginLeft = "";
                J.marginRight = "";
                ...
                K.marginTop = "";
                P.display = "none";
                var C = H - (L - this._containerLeft - this._containerRight);
                var M = (C > 0) ? -C / 2 : 0;
                var I = Q - G;
                var D = (I > 0) ? -I / 2 : 0;
                J.marginLeft = M + "px";
    ...
    if (this._imagePosition != DC.PushButton.IMAGE_POSITION_NONE && (this._customImageLoaded || this._customImagePressedLoaded)) {
        var A = this._pressed ? this._customImagePressed.width : this._customImage.width;
        switch (this._imagePosition) {
            ...
            case DC.PushButton.IMAGE_POSITION_RIGHT:
                if (this._imageElement.style.marginLeft) {
                    B += parseInt(this._imageElement.style.marginLeft, 10)
                }
                if (this._imageElement.style.marginRight) {
                    B += parseInt(this._imageElement.style.marginRight, 10)
                }
                B += C + A;
                break;
            default:
...
;
DC.Indicator = Class.create(DC.DashcodePart, {exposedBindings:["value","onValue","warningValue","criticalValue"],__viewClassName__:"Indicator",init:function() {
    this.base()
},partSetup:function(A) {
    this.base(A);
    ...
    this.imageOffPath = A.imageOff == null ? "Images/IndicatorOff.png" : A.imageOff;
    this.imageOnPath = A.imageOn == null ? "Images/IndicatorOn.png" : A.imageOn;
    this.imageWarningPath = A.imageWarning == null ? "Images/IndicatorWarning.png" : A.imageWarning;
    this.imageCriticalPath = A.imageCritical == null ? "Images/IndicatorCritical.png" : A.imageCritical;
    this.onValue = A.onValue || 0;
    this.warningValue = A.warningValue || 0;
    this.criticalValue = A.criticalValue || 0;
    var C = null;
    var B = null;
    ...
    C.height = "100%";
    C.width = "100%";
    this._indicator.appendChild(B);
    this._image = B;
    this._setBaseImagesPath(A.baseImagesPath || null);
    this.exposedBindings.forEach(function(D) {
        this.addObserverForKeyPath(this, "refresh", D)
    }, this)
},remove:function() {
    var A = this._image.parentNode;
    A.removeChild(this._image)
},refresh:function() {
    ...
    return this.value
},setOnValue:function(A) {
    this.onValue = A
},setWarningValue:function(A) {
    this.warningValue = A
},setCriticalValue:function(A) {
    this.criticalValue = A
},setImageOff:function(A) {
    this.imageOffPath = A;
    this.refresh()
},setImageOn:function(A) {
    this.imageOnPath = A;
    this.refresh()
},setImageWarning:function(A) {
    this.imageWarningPath = A;
    this.refresh()
},setImageCritical:function(A) {
    this.imageCriticalPath = A;
    this.refresh()
},_setValueTo:function(B) {
    this.value = B;
    var A = null;
    if (this.value >= this.criticalValue) {
        A = this.imageCriticalPath
    } else {
        if (this.value >= this.warningValue) {
            A = this.imageWarningPath
        } else {
    ...
    this._image.src = this._baseImagesPath + A
},_setBaseImagesPath:function(A) {
    if (!A) {
        A = ""
    } else {
