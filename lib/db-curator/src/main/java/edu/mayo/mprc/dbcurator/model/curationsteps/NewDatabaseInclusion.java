...
public class NewDatabaseInclusion implements CurationStep {
	...
	public void setHeaderTransform(HeaderTransform trans) {
		this.headerTransformer = trans;
	}

	public HeaderTransform getHeaderTransform(CurationDao curationDao) {
	...
	public StepValidation performStep(CurationExecutor exec) {
		...
		try {
			...
			if (archiveIn != null) {
				try {
					HeaderTransform transform = getHeaderTransform(exec.getCurationDao());
	...
	public StepValidation preValidate(CurationDao curationDao) {
		...
		if (this.url == null) {
			...
			String testURL = (matchedSource == null ? this.url : matchedSource.getUrl());
	...
	public CurationStep createCopy() {
		NewDatabaseInclusion copy = new NewDatabaseInclusion();
	...
	public String simpleDescription() {
		...
		if (lastPathIndex > 0) {
			String filename = url.substring(lastPathIndex + 1).replace(".gz", "").replace(".fasta", "");
	...
	protected static int getIntersectionLength(String s1, String s2) {
		if (s1.contains(s2)) {
