...
public final class SearchEngine implements Serializable, Comparable<SearchEngine>, Cloneable {
	...
	/**
	 * Reverse mapping. This is not reliable and should be used only for transition between old and new Swift versions.
	 *
	 * @param reader  Reader able to provide the parameters.
	 * @param params  Parameters object to fill. If a setting is missing, it would not be set.
	 * @param context Context used for mapping.
	 */
	public void readSearchEngineParameters(Reader reader, SearchEngineParameters params, MappingContext context) {
		...
		final Tolerance peptideTolerance = mapping.mapPeptideToleranceFromNative(context);
		if (peptideTolerance != null) {
			params.setPeptideTolerance(peptideTolerance);
		}

		context.startMapping(ParamName.FragmentTolerance);
		final Tolerance fragmentTolerance = mapping.mapFragmentToleranceFromNative(context);
		if (fragmentTolerance != null) {
			params.setFragmentTolerance(fragmentTolerance);
		}

		context.startMapping(ParamName.MissedCleavages);
		final Integer missedCleavages = mapping.mapMissedCleavagesFromNative(context);
		if (missedCleavages != null) {
			params.setMissedCleavages(missedCleavages);
		}

		context.startMapping(ParamName.Database);
		String shortDatabaseName = mapping.mapSequenceDatabaseFromNative(context);
		if (shortDatabaseName != null) {
			final Curation curation = ParamsInfoImpl.getDatabase(context, shortDatabaseName);
			if (curation != null) {
				params.setDatabase(curation);
			} else {
				LOGGER.error("Unknown database: " + shortDatabaseName);
			}
		}

		context.startMapping(ParamName.Enzyme);
		final Protease protease = mapping.mapEnzymeFromNative(context);
		if (protease != null) {
			params.setProtease(protease);
		}

		context.startMapping(ParamName.VariableMods);
		final ModSet variableMods = mapping.mapVariableModsFromNative(context);
		if (variableMods != null) {
			if (params.getVariableModifications() == null) {
				params.setVariableModifications(variableMods);
			} else {
				params.getVariableModifications().addAll(variableMods.getModifications());
			}
		}

		context.startMapping(ParamName.FixedMods);
		final ModSet fixedMods = mapping.mapFixedModsFromNative(context);
		if (fixedMods != null) {
			if (params.getFixedModifications() == null) {
				params.setFixedModifications(fixedMods);
			} else {
				params.getFixedModifications().addAll(fixedMods.getModifications());
			}
		}

		context.startMapping(ParamName.Instrument);
		final Instrument instrument = mapping.mapInstrumentFromNative(context);
		if (instrument != null) {
		...
		try {
			...
			throw new MprcException("Could not close the input stream when mapping search engine parameters", e);
		}
	}
